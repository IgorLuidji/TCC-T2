    ...
    try {
      if(controller.permission.event){
        if(controller.user.bolManager){
          const { data } = await list(controller.user.id);
...
async function list(user = false,where = {date:{[sequelize.Op.gt]: (new Date(Date.now()))}}, page = 1, limit = 5){
  if(user){
    ...
    const data =  await Event.findAll({ raw: true, where, offset: ((page-1)*limit), limit, order: [['date', 'ASC']] , attributes: ['id','name','date','CongressId','bol_limit','limit']}).then((data) => helpers.formatData(data,true,['date'],['date','dateEnd'])).catch(e => console.log(e))
...
async function listSub(user = false,where = {date:{[sequelize.Op.gt]: (new Date(Date.now()))}}, page = 1, limit = 5){
  if(user){
    ...
    where['id'] = {[sequelize.Op.in]: even};
    const dataSub =  await Event.findAll({ raw: true, where, offset: ((page-1)*limit), limit, order: [['date', 'ASC']] , attributes: ['id','name','date','CongressId','bol_limit','limit']}).then((dataSub) => helpers.formatData(dataSub,true,['date'],['date','dateEnd'])).catch(e => console.log(e))
